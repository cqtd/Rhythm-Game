...
namespace Rhythm.UI
{
	public sealed class SongUI : ScrollerItem, IPointerDownHandler
	{
		...
		{
			Message.Execute<SongInfo>(Event.OnSongPointerDown, _info);
		}
		...
		public void Initialize(SongInfo songInfo)
		{
			...
			BeMusicHeader header = songInfo.Headers.FirstOrDefault();
			...
			if (header != null)
			{
				m_author.SetText(header.artist);
				string path = "file://" + header.parentPath + "/" + header.stageFile;
